...
def registerWhale(addr):
    conn = pymysql.connect("sql", "root", SQL_PWD, "ETH")
    cur = conn.cursor()
...
def logs(tx):
    ...
    system(f'echo {COLOR} transaction occured at block {int(tx["block"], 16)}, {tx["from"]} send {int(tx["value"], 16) / math.pow(10, 18)} '
...
if __name__ == '__main__':
    consumerTx = KafkaConsumer('Tx', bootstrap_servers=['kafka-cluster:9092'], auto_offset_reset='earliest',
                              enable_auto_commit=True,
                              group_id='my-group', value_deserializer=lambda x: loads(x.decode('utf-8')))

    system("echo waiting for messages... ")

    for message in consumerTx:
        tx = message.value


        if int(tx['value'], 16) / 1e+18 > 1:
