...
const fs = require('fs');
let url = 'https://mainnet.infura.io/v3/c61bccacc0c04d068436d27d7b836a51'
var Web3 = require('web3');
var web3 = new Web3(new Web3.providers.HttpProvider(url));
let walletAddress = process.argv[3];
let rawdata = fs.readFileSync(process.argv[2]);
let token_addresses = JSON.parse(rawdata);
// const token_addresses = {'DAI': '0x6b175474e89094c44da98b954eedeac495271d0f' };
...
async function getAllTokenForAddr() {
    dict = {}
    for (const [key, value] of Object.entries(token_addresses)) {
        let contracts = new web3.eth.Contract(abi, value);
        dict[key] = await getBalance(contracts);
    }
    return dict
}


getAllTokenForAddr().then(function (result) {
    console.log(result);
});

// async function getAll(contract) {
//     dict = {}
//     dict['balance'] = await contract.methods.balanceOf(walletAddress).call();
//     dict['symbol'] = await contract.methods.symbol().call();
//     dict['name'] = await contract.methods.name().call();
//     dict['totalSupply'] = await contract.methods.totalSupply().call();
//     return dict;
// }
 
// getBalance(contracts).then(function (result) {
//    console.log(result);
// });
